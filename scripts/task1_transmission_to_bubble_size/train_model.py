...
Target : log1p(bubble_depth)  — maps 0→0, max depth ~91 Mpc/h → ~4.52
...
LOG1P_MAX = 4.53   # log1p(91 Mpc/h) + small buffer; clamp model output here
...
def main():
    ...
    print(f"Total: {N} samples across {n_redshifts} redshift(s)")

    y_log = np.log1p(y_raw)   # target: [0, 4.52]
